...
UCIRAG_HSM$EDAD_UC_IRAG <- factor(UCIRAG_HSM$EDAD_UC_IRAG,
                                     levels = orden_edades,
                                     ordered = TRUE)
...
CASOS_POR_EDAD <- UCIRAG_HSM%>%
  ...
  pivot_wider(names_from = CLASIFICACION_MANUAL, 
              values_from = n, values_fill = 0)  #NECESITAN VISIBILIZAR LOS CASOS INVALIDADOS?

# ----------------------------------------------------------------
# CALCULAR TASA DE LETALIDAD POR GRUPO ETARIO
# ----------------------------------------------------------------
CASOS_TOTALES_POR_EDAD <- UCIRAG_HSM %>%
...
FALLECIDOS_POR_EDAD <- UCIRAG_HSM %>%
  filter(!is.na(EDAD_UC_IRAG)) %>%
  group_by(EDAD_UC_IRAG) %>%
